...
import { transBankPayment } from "@/actions";
...
export const TransBankButton = async ({ orderId, amount }: Props) => {

    const createOrder = async(orderId: string, amount: number) => {

        const data = await transBankPayment(orderId, amount);
  ...
  return (
    <div className="flex items-center w-full">
        <button
        onClick={ () => createOrder(orderId, amount) }
        className="flex items-center w-full text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:focus:ring-gray-800 dark:bg-white dark:border-gray-700 dark:text-gray-900 dark:hover:bg-gray-200 me-2 mb-2"
        >
        <svg
            aria-hidden="true"
